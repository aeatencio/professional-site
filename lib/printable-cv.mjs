 ...
 *
 * Boundary: the built HTML for every printable route (`/cv/`, `/cv/letter/`,
 * `/es/cv/` and `/es/cv/letter/`), including head, metadata, `@page`,
 * on-screen chrome and `#cv-main`; every local stylesheet those documents
 * load; every local font/image those documents or stylesheets reference;
 * `Astro.site`; and the `Page.printToPDF` options used for each paper size.
 *
